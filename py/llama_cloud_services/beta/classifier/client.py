...
class ClassifyClient:
    ...
    async def classify_file_ids(
        ...
        if raise_on_error and classify_job_with_status.status == StatusEnum.ERROR:
            raise ValueError(f"Error classifying files under job ID {classify_job_with_status.id}")
    ...
    async def classify_file_path(
        ...
        return await self.classify_file_ids(rules, [file.id], parsing_configuration, raise_on_error)
    ...
    async def classify_file_paths(
        ...
        return await self.classify_file_ids(rules, [file.id for file in files], parsing_configuration, raise_on_error)
